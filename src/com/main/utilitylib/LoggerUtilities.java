...
public class LoggerUtilities{
	...
	public BufferedWriter getDashboardLog() throws IOException{
		fwsummary = new FileWriter(getLogFile("LogDashboard.log").getAbsolutePath());
		logsummary =  new BufferedWriter(fwsummary);
		return logsummary;
	}
	
	//Generate log file for current test batch
	public BufferedWriter generateLogFile(String testsuite) throws IOException{
		 timestamp = getTimestamp().toString().replace(":", "_");
		    logfile = getLogFile(testsuite+"_testbatch_"+timestamp+".log").getAbsoluteFile();
	...
	public Error lognThrow(BufferedWriter logger, Exception e) throws Exception{
		...
		logSummary(getTimestamp()+"	ERROR: "+e.getMessage()+"\n"+getTimestamp()+"	#XX# TEST FAILED!!! #XX#");
